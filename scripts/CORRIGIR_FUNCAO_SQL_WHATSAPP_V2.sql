-- ============================================
-- CORREÇÃO V2: Criar função criar_orcamento_e_notificar
-- COM FIX PARA CONSTRAINT UNIQUE
-- Execute este script no Supabase SQL Editor
-- ============================================

-- ============================================
-- 0. CRIAR CONSTRAINT UNIQUE (se não existir)
-- ============================================
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'orcamentos_campanhas_unique_orcamento_campanha'
  ) THEN
    ALTER TABLE orcamentos_campanhas
    ADD CONSTRAINT orcamentos_campanhas_unique_orcamento_campanha
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE '⚠️ Erro ao criar constraint (pode já existir): %', SQLERRM;
END $$;
...
-- (COM VERIFICAÇÃO PARA EVITAR DUPLICATAS)
-- ============================================
...
BEGIN
  ...
  -- (COM VERIFICAÇÃO MANUAL PARA EVITAR DUPLICATAS)
  -- ============================================
...
SELECT * FROM criar_orcamento_e_notificar(
  jsonb_build_object(
    'nome_cliente', 'Teste WhatsApp V2',
    'email_cliente', 'testev2@whatsapp.com',
    'telefone_cliente', '11987654321',
    'whatsapp', '11987654321',
    'origem_completo', 'Rio de Janeiro, RJ',
...
-- ============================================
-- 7. VERIFICAR SE HÁ CAMPANHAS EM SP
-- ============================================
...
SELECT
  ...
  c.data_fim,
  CASE
    WHEN c.ativo = false THEN '❌ Inativa'
    WHEN c.participa_cotacao = false THEN '❌ Não participa cotação'
    WHEN c.data_fim < CURRENT_DATE THEN '❌ Expirada'
    WHEN h.nome_exibicao IS NULL THEN '❌ Sem nome'
    ELSE '✅ OK'
  END as status
FROM hotsites h
INNER JOIN campanhas c ON c.hotsite_id = h.id
INNER JOIN cidades ci ON h.cidade_id = ci.id
WHERE ci.estado = 'SP'
ORDER BY c.ativo DESC, c.participa_cotacao DESC;
